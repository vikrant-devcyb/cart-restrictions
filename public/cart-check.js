...
async function validateCartBeforeCheckout() {
    ...
    try {
        ...
        const url = `/apps/single-location-checkout-control?shop=${shop}&variant_ids=${variantIds}`;
...
async function unsubscribeCustomer(email, shop) {
    ...
    try {
        const url = `/apps/single-location-checkout-control?action=unsubscribe&email=${encodeURIComponent(email)}&shop=${encodeURIComponent(shop)}`;
        const response = await fetch(url);
        const data = await response.json();
        if (data.message) {
